...
export type Struct = CustomStruct<any> | SimpleStruct | ObjectStruct;

export type PrimitiveTypeMap = { [K in TextType]: string } & { [K in NumericType | FloatType]: number } & {
...
export type PrimtiveEncoder<T> = (value: OrArray<T>, isArray: boolean, arrayLength: number) => number[];
