...
export type PrimitiveTypeMap = {
    [K in TextType]: string;
} & {
    [K in NumericType | FloatType]: number;
} & {
    [K in BigIntType]: bigint;
};
type DecodePrimitive<T extends SimpleStruct> = T extends PrimitiveType ? PrimitiveTypeMap[T] : T extends `${infer PT extends PrimitiveType}[${number | ""}]` ? PrimitiveTypeMap[PT][] : never;
type DecodeTuple<T extends Struct[], Collector extends unknown[] = []> = T extends [
    infer Strct extends Struct,
    ...infer Rest extends Struct[]
] ? DecodeTuple<Rest, [...Collector, Data<Strct>]> : Collector;
export type Data<Strct extends Struct> = Strct extends SimpleStruct ? DecodePrimitive<Strct> : Strct extends CustomStruct<infer CS> ? CS : Strct extends Struct[] ? DecodeTuple<Strct> : Strct extends Struct ? {
...
export type PrimtiveEncoder<T> = (value: OrArray<T>, isArray: boolean, arrayLength: number) => Uint8Array<ArrayBuffer>;
export type PrimtiveDecoder<T> = (buffer: Uint8Array<ArrayBuffer>, offset: number, isArray: boolean, arrayLength: number) => StructDecodeResult<OrArray<T>>;
export type PrimitiveEncoderMap = {
    [K in PrimitiveType]: PrimtiveEncoder<DecodePrimitive<K>>;
};
export type PrimitiveDecoderMap = {
    [K in PrimitiveType]: PrimtiveDecoder<DecodePrimitive<K>>;
};
