import { encodingError, EncodingError } from "./error.js";
import { getStructType, StructType } from "./struct.js";
import { destructureSimpleStruct, encodeNumber, getStringCodePoints, sortObjectEntries, } from "./utils.js";
const encodeNumberStruct = (bitLength, isFloat, isSigned, value, isArray, arrayLength) => {
    ...
    let methodName;
    {
        if (isFloat) {
            if (bitLength === 32)
                ...
                throw encodingError(`Unexpected bit length of ${bitLength} for floats.`);
        }
        else {
...
const charGuard = (codepoint) => {
    if (codepoint < 256)
        return codepoint;
    throw encodingError(`char codepoint must not exceed an 8-bit binary representation, got ${codepoint.toString(2).length} bits.`, codepoint);
};
const encodeOneChar = (char) => {
    ...
    char: (value, isArray, arrayLength) => {
        if (!isArray) {
            if (typeof value === "string")
                return new Uint8Array([encodeOneChar(value)]);
...
export const encode = (struct, payload) => {
    const pairings = [[struct, payload]];
    const arrays = [];
    let totalLength = 0;
    while (pairings.length) {
        ...
        switch (getStructType(pair[0])) {
            case StructType.Simple: {
                const ds = destructureSimpleStruct(pair[0]);
            ...
            case StructType.Object: {
                const structEntries = sortObjectEntries(Object.entries(pair[0]));
            ...
            case StructType.Tuple: {
                const [struct, data] = pair;
                pairings.unshift(...struct.map((s, i) => [s, data[i]]));
                break;
            }
            case StructType.Custom: {
                const result = pair[0].encode(pair[1]);
                totalLength += result.length;
                arrays.push(result);
                break;
            }
